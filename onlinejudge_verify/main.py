...
try:
    ...
    print(
        "Due to a known bug, the online-judge-tools is not yet properly installed. Please re-run $ pip3 install --force-reinstall online-judge-api-client"
    )
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--config-file",
        default=onlinejudge_verify.config.default_config_path,
        help='default: ".verify-helper/config.toml"',
    )
...
def subcommand_run(
    paths: List[pathlib.Path], *, timeout: float = 600, tle: float = 60, jobs: int = 1
) -> onlinejudge_verify.verify.VerificationSummary:
    """
    ...
    does_push = (
        ...
        and os.environ.get("GITHUB_REF", "").startswith("refs/heads/")
    )  # NOTE: $GITHUB_REF may be refs/pull/... or refs/tags/...
    if does_push:
    ...
    try:
        with onlinejudge_verify.marker.get_verification_marker() as marker:
            return onlinejudge_verify.verify.main(
                paths, marker=marker, timeout=timeout, tle=tle, jobs=jobs
            )
    finally:
...
def push_documents_to_gh_pages(
    *, src_dir: pathlib.Path, dst_branch: str = "gh-pages"
) -> None:
    # read config
    ...
    url = "https://{}@github.com/{}.git".format(
        os.environ["GH_PAT"], os.environ["GITHUB_REPOSITORY"]
    )
    logger.info("GITHUB_REPOSITORY = %s", os.environ["GITHUB_REPOSITORY"])
    ...
    subprocess.check_call(
        ["rm", ".verify-helper/.gitignore"]
    )  # required, to remove .gitignore even if it is untracked
...
def _delete_gitignore() -> None:
    ...
    try:
        ...
        gitignore_checked_in = (
            subprocess.run(
                ["git", "ls-files", "--error-unmatch", str(gitignore_path)], check=False
            ).returncode
            == 0
        )
        if not gitignore_checked_in:
            return
        logger.warning(
            "file %s exists in this Git repository. It should not be checked in.",
            str(gitignore_path),
        )
...
def main(args: Optional[List[str]] = None) -> None:
    ...
    if parsed.subcommand == "all":
        _delete_gitignore()
        generate_gitignore()
        summary = subcommand_run(
            paths=[], timeout=parsed.timeout, tle=parsed.tle, jobs=parsed.jobs
        )
        ...
        generate_gitignore()
        summary = subcommand_run(
            paths=parsed.path, timeout=parsed.timeout, tle=parsed.tle, jobs=parsed.jobs
        )
        summary.show()
        if not summary.succeeded():
            sys.exit(1)

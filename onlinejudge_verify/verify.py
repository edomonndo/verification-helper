    ...
    def show(self) -> None:
        if self.failed_test_paths:
            ...
            for path in self.failed_test_paths:
                logger.error(
                    "failed: %s", str(path.resolve().relative_to(pathlib.Path.cwd()))
                )
        else:
...
def verify_file(
    ...
    # download test cases
    directory = (
        pathlib.Path(".verify-helper/cache") / hashlib.md5(url.encode()).hexdigest()
    )
    if not (directory / "test").exists() or list((directory / "test").iterdir()) == []:
        ...
        try:
            ...
            traceback.print_exc()
            if isinstance(
                problem, onlinejudge.service.yukicoder.YukicoderProblem
            ) and not os.environ.get("YUKICODER_TOKEN"):
    ...
    for environment in language.list_environments(path, basedir=pathlib.Path.cwd()):
        ...
        try:
            ...
            execute = " ".join(
                environment.get_execute_command(
                    path, basedir=pathlib.Path.cwd(), tempdir=directory
                )
            )  # TODO: use shlex.join added in Python 3.8
        except Exception:
            traceback.print_exc()
            return False
        ...
        if isinstance(
            problem, onlinejudge.service.library_checker.LibraryCheckerProblem
        ):
